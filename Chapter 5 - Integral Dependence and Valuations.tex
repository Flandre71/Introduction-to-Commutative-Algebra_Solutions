...
\begin{exercise}
    ...
    (Elimination of terms to make the elements integral is easy, as we can nest
    those polynomials by terms.)
\end{exercise}

\begin{exercise}
    Suppose $\exists x$ integral over $A$ in $B\backslash A$,
    i.e.$\exists a_0,\ldots ,a_{n-1}\in A,\text{s.t. }$
    $$
    x^{n}+a_{n-1}x^{n-1}+\ldots a_1x+a_0=0
    $$
    Hence
    $$
    x(x^{n-1}+a_{n-1}x^{n-2}+\ldots +a_1)=-a_0\in A
    $$
    By the multiplicative closeness of $B\backslash A$,
    $$
    x^{n-1}+a_{n-1}x^{n-2}+\ldots +a_1\in A
    $$
...
\begin{exercise}
    ...
    \begin{enumerate}[label=({\roman*})]
        \item 
            \emph{(The hint just proved everything, there's nothing else I could say.)}
        \item 
            Similarly, we can think of a way to split $f,g$ without the
            assisstant of algebraically closed field, as its existance requires
            $B$ being integral domain:

            Notice $f,g$, as polynomials, have finite degree. Therefore, such
            can be achieved by having field extension finitely many times.
    \end{enumerate}
    \qed
\end{exercise}

\begin{exercise}
        \emph{(I failed, but the hint proved everything again.)}
\end{exercise}
